...
daily_w_mandates_df <- read_csv("data/output/daily_data_with_other_NPI_df_produced_2021-02-12 15.14.csv") # data until Sep 27th


early_wave_states <- c('NY', 'NJ', 'MA', 'CT', 'MI', 'DC', 'RI', 'IL', 'WA', 'PA', 'GA', 'VT', 'MD', 'FL', 'LA')

# FLAG TO PICK EARLY AND NOT EARLY STATES
# daily_w_mandates_df <- subset(daily_w_mandates_df, state %in% early_wave_states)
daily_w_mandates_df <- subset(daily_w_mandates_df, !state %in% early_wave_states)
...
# ggsave('plots/daily_event_study_state_level_mandates_first_wave.pdf',
# ggsave('plots/daily_event_study_state_level_mandates_NOT_first_wave.pdf',
# width = 8,
# height = 10,
# dpi = 300)
