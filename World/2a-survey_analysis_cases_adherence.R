...
survey_import_df <- read_csv("survey_mobility_cases_wave10.csv")
...
mask_plot_coef_df_cases <- subset(all_coef_df_cases, term == 'distancing_norms_wear_a_face_mask_or_covering')
...
ggplot(data=mask_plot_coef_df_cases, aes(x=wave, y=estimate)) +
  ...
  xlab("Survey Wave") +
  ylab("Estimaged Change in Cases Associated 
...
Oct 25"
  ...
  theme_linedraw()+  
  ggtitle("Effect of Community Mask Adherance on COVID-19 Cases")+
  theme(plot.title = element_text(hjust = 0.5))


ggsave('plots/compliance_cases_distancing_norms_wear_a_face_mask_or_covering.pdf',
       width = 7,
       height = 4,
       dpi = 300)
